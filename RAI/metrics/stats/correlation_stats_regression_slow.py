...
class CorrelationStatRegressionSlow(MetricGroup, class_location=os.path.abspath(__file__)):
    ...
    def compute(self, data_dict):
        ...
        self.metrics["siegel_slopes"].value = calculate_per_mapped_features(
            scipy.stats.siegelslopes, map, features, scalar_data, data.y
        )
        self.metrics["theil_slopes"].value = calculate_per_mapped_features(
            scipy.stats.theilslopes, map, features, scalar_data, data.y
        )
