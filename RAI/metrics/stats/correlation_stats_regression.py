...
class CorrelationStatRegression(MetricGroup, class_location=os.path.abspath(__file__)):
    ...
    def compute(self, data_dict):
        ...
        self.metrics["pearson_correlation"].value = calculate_per_mapped_features(
            scipy.stats.pearsonr, map, features, data.scalar, data.y, to_array=False
        )
        self.metrics["spearman_correlation"].value = calculate_per_mapped_features(
            scipy.stats.spearmanr, map, features, data.scalar, data.y, to_array=False
        )
