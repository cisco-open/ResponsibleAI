...
for cat in metrics:
    print("\n", cat, " metrics:")
    for group in metrics[cat]:
        for metric in metrics[cat][group]:
            if "type" in info[group][metric]:
                if info[group][metric]["type"] in ("numeric", "vector-dict", "text"):
                    print(group, metric, metrics[cat][group][metric])
